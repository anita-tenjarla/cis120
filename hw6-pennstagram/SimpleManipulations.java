...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
    	...
    	Pixel[][] edited = new Pixel[w + (2 * borderWidth)][h + (2 * borderWidth)];
    	
    	for (int x = 0; x < w + borderWidth + borderWidth; x++){ 
    		for (int y = 0; y < h + borderWidth + borderWidth; y++) {
    			if (((x < borderWidth) || (x >= w + borderWidth)) || 
    					((y < borderWidth) || (y >= h + borderWidth))) {
    				edited[x][y] = borderColor; 
    			} else {
    				edited[x][y] = source[x-borderWidth][y-borderWidth];
    			}
    		}
    	}
    	
    	return new PixelPicture(edited); 
    }
     ...
     * http://download.oracle.com/javase/6/docs/api/java/lang/Math.html#round%28double%29
     *
   ...
   public static PixelPicture scaleColors(
       ...
       for (int x = 0; x < w; x++) {
           for (int y = 0; y < h; y++) {
              ...
              int r = p.getRed();
               int g = p.getGreen();
               int b = p.getBlue();
               int ravg = (int) Math.round(r * rfactor);
               int gavg = (int) Math.round(g * gfactor);
               int bavg = (int) Math.round(b * bfactor);
    ...
    public static PixelPicture alphaBlend(
        ...
        if ((pic.getWidth() != f.getWidth()) || pic.getHeight() != f.getHeight()) {
        	return pic; 
        } else {
        	Pixel[][] bmp = pic.getBitmap();
            ...
            for (int x = 0; x < pic.getWidth(); x++) {
                for (int y = 0; y < pic.getHeight(); y++) {
                   ...
                   int g2 = q.getGreen();
                   int b2 = q.getBlue();
                   int ravg = weightedAverage(alpha, r1, r2);
                   int gavg = weightedAverage(alpha, g1, g2);
                   int bavg = weightedAverage(alpha, b1, b2);
                   bmp[x][y] = new Pixel(ravg, gavg, bavg);
                }
            }
          return new PixelPicture(bmp);
    ...
    *
    */
    /** Adding dark edges to an image to draw interest
     * to the center.
     *
