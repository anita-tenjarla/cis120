...
let checkbox (init: bool) (s: string)
  ...
  let current_state = { contents = init } in
  let listener = { contents = [] } in
  
  let w : widget = {
    
    repaint = (fun (g: Gctx.gctx) ->
        if current_state.contents 
        then Gctx.draw_string (Gctx.with_color g Gctx.red) (0,0) s
        else Gctx.draw_string (Gctx.with_color g Gctx.black) (0,0) s
        );
    ...
    handle =   
      (fun (g: Gctx.gctx) (e: Gctx.event) ->
            if true then (
              begin match Gctx.event_type e with
              | Gctx.MouseDown -> current_state.contents <- 
              (not current_state.contents)
              | _ -> ()
              end ;
              List.iter (fun h -> h current_state.contents) 
              listener.contents; )
            else ());
  ...
  let controller : bool value_controller = {
    
    add_change_listener = (fun (cl: bool -> unit) -> 
      listener.contents <- cl :: listener.contents);
    get_value = (fun () -> current_state.contents); 
    set_value = (fun (cs: bool) -> current_state.contents <- cs); 
    } 
...
(*slider that controls thickness of line*)
(*it doesn't work but here's my attempt*)
(*really can't find the bug but hopefully this counts for something *)

let tbar (init: int) (s: string) (t: int)
    : widget * int value_controller =
      
  let current_state = { contents = init } in
  let listener = { contents = [] } in
  let box = (border (space (255, 10))) in
  
  let nc : int value_controller = {
    
    add_change_listener = (fun (cl: int -> unit) -> 
      listener.contents <- cl :: listener.contents);
    get_value = (fun () -> current_state.contents); 
    set_value = (fun (cs: int) -> current_state.contents <- cs);
  ...
  let wid : widget = {
    repaint = (fun (g: Gctx.gctx) ->
        Gctx.draw_rect g (10, 10) (255, 10);
        Gctx.draw_string (Gctx.with_color g Gctx.black) (0,10) s; 
        let newg = Gctx.with_thickness g t in
        Gctx.fill_rect newg (current_state.contents, 10) (10, 10));    

    handle =   
      (fun (g: Gctx.gctx) (e: Gctx.event) ->
            if event_within g e (current_state.contents, 10) then (
            let position = Gctx.event_pos e g in 
              begin match Gctx.event_type e with
              | Gctx.MouseDown -> nc.set_value (fst position)
              | Gctx.MouseDrag -> nc.set_value (fst position)
              | _ -> ()
              end ; 
             List.iter (fun h -> h current_state.contents) listener.contents;)
            else () ; ) ;
