...
public final class Broadcast {
    ...
    /**
     * Enqueue a response to be sent to a given user ID.
     ...
     * {@link Command} is accepted by the server and should be relayed to the
     * appropriate clients.
     * @param command The {@link Command} whose effects to broadcast
     * @param recipients The set of user names of clients who should receive
     *                   the broadcast from the server
    ...
    /**
     * Creates a {@code Broadcast} for the case where a client's {@link
     * Command} is invalid, and the client should be informed.
     * @param command The command which caused the error
     * @param error The {@link ServerError} that the command caused
    ...
    /**
     * Creates a {@code Broadcast} for the case when a user first connects to
     ...
     * @param recipients A set of nicknames of clients who should be informed
     *                   of the user's disconnection. Should not include the
     *                   user who disconnected.
     * @return A {@code Broadcast} representing the response to send
     */
    public static Broadcast disconnected(String user, Set<String> recipients) {
    ...
    /**
     * A specialized method for creating a {@code Broadcast} in the event that
     * a user is added to a channel as the result of a {@link JoinCommand} or
     * {@link InviteCommand}. The resulting Broadcast informs the relevant
     * client of the nicknames of all other users in the channel.
     * @param command The command issued by the client
     * @param recipients A set of nicknames of the other users in the channel
     ...
     * @throws IllegalArgumentException if {@code command} is not an instance
     * of {@link JoinCommand} or {@link InviteCommand}
     */
    public static Broadcast names(Command command, Set<String> recipients, String owner) {
        ...
        String namesResponse = String.format(":%s NAMES %s :%s", userToAdd, channelName, namesPayload);
    ...
    /**
     * Associates the stored responses with the user IDs of the recipients. This
     * function will be called by the {@link ServerBackend} before dispatching
     * the {@code Broadcast}; you should not call this yourself.
     * @param model A class conforming to {@link ServerModelApi} which can be
     *              used to look up user IDs.
     * @return a mapping from user ID to a list of response strings that
     * should be delivered by the {@link ServerBackend}
     */
    ...
    /**
     * Generates a payload for a {@code NAMES} response for some channel, given
     * the nickname of the channel's owner, and a collection of the
     * nicknames of the users in the channel (including the owner).
     ...
     *         {@code nicks} does not contain {@code owner}
     */
    private static String createNamesPayload(String owner, Collection<String> nicks) {
        if (owner == null || nicks == null || !nicks.contains(owner)) {
        	if (owner == null) System.out.println("owner null");
        	if (nicks == null) System.out.println("nicks null");
        	if (!nicks.contains(owner)) System.out.println("owner not present");
